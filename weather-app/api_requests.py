...
def get_location_area_key_for_city(city: City, token: str) -> str:
    latitude, longitude = city.info["latitude"], city.info["longitude"]
...
def get_forecast(city: City, time_range : TimeRange, token: str) -> dict:

    if city.uuid in cities_with_location_key:
        city_location_key = cities_with_location_key[city.uuid]
    else:
        city_location_key = get_location_area_key_for_city(city, token)
        cities_with_location_key[city.uuid] = city_location_key
